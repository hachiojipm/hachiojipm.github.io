...
実際のプロジェクトにおいては5.10以降の機能も使えるものは積極的に使っているのでその辺りの話を書きます。

## `say`: 改行付きprint (5.10)

`print`してその後に改行を入れてくれる関数です。地味に便利です。ワンライナーでも -E スイッチで使えるようになるので近年は-eを使わずに専ら-Eです。
...
あまり使うことはないのですが、`...`という文字列がサンプルコードとしてvalidであったりするのがちょっと便利。もしくはモジュールなんかでもこの"メソッドは将来的には用意するつもりだよ"的なことを示すのに使ったりできます。
